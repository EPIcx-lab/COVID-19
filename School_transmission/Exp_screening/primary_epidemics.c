...
int main(int argc, char *argv[]){
  ...
  FILE *fp12;
  char outfile12[dim_str];
  
  FILE *fp13;
  char outfile13[dim_str];
  ...
  sprintf(introfilestudents,"./data_in/regular_intro_stud_rr0.93_Rhone.txt");
  sprintf(introfileteachers,"./data_in/regular_intro_teac_rr0.93_Rhone.txt");
  ...
  for (jj=0; jj<3; jj++){
  	for(ii=0; ii<7; ii++){
  		//fprintf(stderr, "%f\t", introst[jj].weeks[ii]);
  	}
  	//fprintf(stderr, "\n");
  }
  ...
  for (jj=0; jj<3; jj++){
  	for(ii=0; ii<7; ii++){
  		//fprintf(stderr, "%f\t", introtc[jj].weeks[ii]);
  	}
  	//fprintf(stderr, "\n");
  }
  ...
  for(ii=0; ii<sigma_st.nweeks; ii++){
  	//fprintf(stderr, "%d\t %f\t %f\t %f\n", ii, sigma_st.novax[ii], sigma_st.twodoses[ii], sigma_st.booster[ii] );
  }
  ...
  for(ii=0; ii<rbeta_st.nweeks; ii++){
  	//fprintf(stderr, "%d\t %f\t %f\t %f\t %f\t %f\t %f\n", ii, rbeta_st.novax_p[ii], rbeta_st.novax_c[ii], rbeta_st.twodoses_p[ii], rbeta_st.twodoses_c[ii],  rbeta_st.booster_p[ii], rbeta_st.booster_c[ii] );
  } 
  ...
  for(ii=0; ii<psub_st.nweeks; ii++){
  	//fprintf(stderr, "%d\t %f\t %f\t %f\n", ii, psub_st.novax[ii], psub_st.twodoses[ii], psub_st.booster[ii] );
  } 
  ...
  param.shape_latent = 3; 
  param.shape_prodro = 1; 
  param.shape_infect = 6; 
  param.shape_rp = 4;
  ...
  // INPUT FROM COMMAND LINE
   ...
   if(argc!=1){
      ...
      narg++;
      param.beta = atof(argv[narg]);         //transmission rate/min		
      narg++;
      ...
      narg++;
      days_isolation = atoi(argv[narg]);      // Days of isolation 
      narg++;
      
      
      ntimes= atoi(argv[narg]);      // test frequency n-times
      narg++;
      nweek=atoi(argv[narg]);      // per week
      narg++;
  ...
  //fprintf(stderr, "---------- n rem stud: %d\t n rem ad: %d ------------\n", n_removed_stud, n_removed_teac);
  ...
  param.mu_rp1=1.0/(param.tau_rp-param.delta_q); // exit rate from R+ rescaled for the time spent in isolation
  ...
  //fprintf(stderr, "SCHOOL HOURS %d\t  %d\n", val_start, val_end );
  ...
  ////fprintf(stderr, "%f\t %d\t %d\n", param.alpha, param.deltar, param.delay);
  ...
  sprintf(outfile12, "%srscreen_%s.txt", dir_out, str_param);
  fp12=fopen(outfile12, "w");
  
  sprintf(outfile13, "%sregid_%s.txt", dir_out, str_param);
  fp13=fopen(outfile13, "w");
